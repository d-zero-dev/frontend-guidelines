...
セレクタの形式（BEM 記法のフラットセレクタ、記述順など）は Stylelint が強制するため、警告に従って修正すればよい。
...
lint（Stylelint / Prettier）が担保する事項は人力で確認せず、lint が実行されているかだけを確認する。

- `general/` に安易な追加がないか（コンポーネントで定義できるものが混ざっていないか）
- 1 ファイルに複数コンポーネントが定義されていないか
