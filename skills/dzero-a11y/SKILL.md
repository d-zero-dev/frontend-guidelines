...
- WCAG 2.2 の 4 原則（知覚可能・操作可能・理解可能・堅牢性）に基づいてマシンリーダブルにマークアップする
- 各要素のコンテンツモデルを理解し、コンポーネント・機能・文脈によって最適なタグを選択する
- WAI-ARIA は WAI-ARIA 1.2 を基本に、ブラウザ・支援技術の実装状況を鑑みて判断する
...
- 見出しレベルをスキップしない。`section` 内で `h1` を繰り返さない（Markuplint が警告するが、レベル設計自体は実装時に判断する）
